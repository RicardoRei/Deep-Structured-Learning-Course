...
def main():
    ...
    # Init. Model
    perceptron = StructuredPerceptron(train_x[0].shape[1], len(letter2ix), pairwise=True)

    # Train Model
    train_accuracy, dev_accuracy = perceptron.train(
        train_x, [[letter2ix[ix] for ix in train_y[i]] for i in range(train_y.shape[0])], 
        dev_x, [[letter2ix[ix] for ix in dev_y[i]] for i in range(dev_y.shape[0])], epochs=20
    )
    
    print ("Test Accuracy: {}".format(perceptron.evaluate(
        test_x, [[letter2ix[ix] for ix in test_y[i]] for i in range(test_y.shape[0])]))
    )
