...
class DeepFeedForwardNN(nn.Module):
	...
	def __init__(self, input_size, n_classes, layers=1, dropout=0.0, activation_func="Sigmoid"):
		"""
	...
	def forward(self, x):
		...
		layeri_out= self.activation(linear0(torch.FloatTensor(x).view(1,-1)))
		...
		return F.softmax(self.dropout(self.output_linear(layeri_out)), dim=1)
...
def main():
	...
	dff_nn, train_accuracy, dev_accuracy = train(
		dff_nn, train_x, train_y, dev_x, dev_y, 
