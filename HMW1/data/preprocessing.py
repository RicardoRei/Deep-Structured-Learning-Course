...
def main(kernel=False):
	...
	if kernel:
		...
		joblib.dump((np.array(train_x), np.array(train_y)), 'kernel_train.pkl')
		joblib.dump((np.array(dev_x), np.array(dev_y)), 'kernel_dev.pkl')
		joblib.dump((np.array(test_x), np.array(test_y)), 'kernel_test.pkl')
	else:
...
if __name__ == '__main__':
	...
	parser.add_argument("--kernel", type=bool, default=False, help="Flag to apply the pairwise multiplication transformation to the original pixels.")
